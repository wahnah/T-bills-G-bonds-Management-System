<template>
  ...
  <div v-if="pagination.total > 0">
    <div class="w-full overflow-hidden rounded-lg shadow-xs">
          ...
          <th class="px-4 py-3">Expected Coupon Income</th>
          <th class="px-4 py-3">Resell</th>
          <th class="px-4 py-3">Download Award note</th>
        </tr>
      </thead>
      <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
        <tr v-for="purchase in purchases" :key="purchase.lot_id">
          ...
          <td class="px-4 py-3 text-sm text-gray-600">{{ purchase.coupon }}</td>
          ...
          <td class="px-4 py-3 text-sm text-gray-600">ZMK{{ purchase.totalCouponPay }}</td>
          <td class="px-4 py-3">
  ...
  <div v-else class="text-gray-600">
    You have no Government Bond purchases.
  </div>
</div>
</template>
...
export default {
    ...
    computed: {
    total() {
      return this.purchases.reduce((accumulator, purchase) => accumulator + parseFloat(purchase.price), 0);
    },
  },
    mounted() {
        this.getPurchases();
